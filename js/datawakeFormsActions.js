...
$('#readView').on('click', function () {
    ...
    if ($formulario != null) {
        ...
        formFields = $formulario.querySelectorAll(formActionsInputTypes);
        fPreencheForm(nFields, formFields, dados, true)
        if ($nameForm2 != '') {
            formFields = formCiclosTemp.querySelectorAll(formActionsInputTypes);
...
$('#editRec').on('click', function () {
    ...
    if ($formulario != null) {
        ...
        $('#buttonBarForm ').empty();
        $('#buttonBarForm ').append('<button class="btn btn-primary" id="confirmButton" type="submit" onclick="javascript:fValidForms(2)">Confirmar</button>');
        $('#buttonBarForm ').append('<button class="btn btn-danger"   data-bs-toggle="modal" type="button" id="cancEdit" onclick="javascript:fcancEdit()">Cancelar</button>');
...
$('#addNew').on('click', function () {
    ...
    if ($formulario != null) {
        ...
        dados = data;
        if ($nameForm1 === "#formUsuariosV2" && dados.length) {
            dados = fViewmGroup(dados.length > 0 ? dados['id'] : "");

        }
        if ($nameForm1 === "#formUsuarios") {
            dados = fViewDataUser(dados.length > 0 ? dados['id'] : "");
        }
        document.getElementById("titleForm").innerHTML = $nameTitleForm;
        formFields = $formulario.querySelectorAll(formActionsInputTypes);
        ...
        if ($nameForm1 === "#formGpAcesso") {
            dados = fViewPermGroup(dados.length > 0 ? dados['id'] : "");
        }

        if ($nameForm2 != '') {
            formFields = $formCiclosTemp.querySelectorAll(formActionsInputTypes);
            fPreencheForm(nFields, formFields, dados.length > 0 ? dados[$nameForm2.substring(1, $nameForm2.length)] : [], false);
        }

        $('#buttonBarForm ').empty();
        $('#buttonBarForm ').append('<button class="btn btn-primary" id="confirmButton" type="button" onclick="javascript:fValidForms(1)">Confirmar</button>');
        $('#buttonBarForm ').append('<button class="btn btn-danger" data-bs-toggle="modal"  type="button" id="cancEdit" onclick="javascript:fcancEdit()">Cancelar</button>');
